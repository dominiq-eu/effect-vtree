...
function slideView(slide: Slide): VTreeNode {
  return (
    <g
      // The key belongs to the `<g>` so each labeled shape keeps its DOM
      // identity while its SVG attributes change.
      key={slide.id}
    ...
    >
      ...
      />
      <text
        class="pointer-events-none text-sm font-bold transition-all duration-700 ease-in-out select-none"
        x={slide.x}
        y={slide.y + 5}
        text-anchor="middle"
        fill="#020617"
...
function view(currentOffset: number): VTreeNode {
  ...
  return (
    <section class="flex min-h-screen items-center justify-center px-6 py-12 font-sans">
      <div class="w-full max-w-2xl rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/50">
        ...
        <p class="mt-3 text-sm leading-6 text-slate-300">
          Click the graphic to patch new SVG attributes. Browser CSS transitions
          animate the reconciled DOM.
        </p>
        <button
          class="mt-6 rounded-xl bg-cyan-400 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300"
          onClick={rotateSlides}
        >
          Rotate
        </button>
        <svg class="mt-6 w-full rounded-2xl bg-slate-950" viewBox="0 0 420 300">
